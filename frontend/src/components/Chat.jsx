...
const Chat = () => {
    ...
    const askDetails = async (Time, DocName, Email, Date) => {
                        ...
                        if (matchedDoctor.length !== 0) {
                            ...
                            const doctorInfoMessage = (
                                <div className="flex flex-col px-5 m-1">
                                    ...
                                    {matchedDoctor.map((doctor, index) => (
                            ...
                            const nodoc = await axios.post('http://127.0.0.1:8000/gemini_data', {
                                Text: "Hey Med return in text format that user has either given incomplete query or query is not clear and if had provide the necessary and no doctor is found in database return that you are trying to collaborate to /Specializatio/ in /Location/."
                            });
                        ...
                        const availTimes = (
                            <div>
                                ...
                                <div>
                                    ...
                                    {selectedDoc.TimeSlot.some((dateSlot) => dateSlot.TimeSlots.some((timeSlot) => timeSlot.isAvailable)) ?
                                        selectedDoc.TimeSlot.map((dateSlot, dateIndex) => (
                                            <div key={dateIndex} className='flex w-[600px] m-5'>
                                                <p><strong>Date:</strong> {dateSlot.Date}</p>
                                                {dateSlot.TimeSlots.some((timeSlot) => timeSlot.isAvailable) ?
